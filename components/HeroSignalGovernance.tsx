"use client";

import type { HeroSignalData } from "@/app/lib/types/dashboard";

const DISPLAY = "'Plus Jakarta Sans', 'DM Sans', sans-serif";
const MONO = "'DM Sans', ui-monospace, monospace";
const TEAL = "#1D9E75";
const TEAL_BRIGHT = "#27C893";
const AMBER = "#EF9F27";
const TEXT = "#E8EDF4";
const TEXT_MUTED = "#8BA0BC";
const TEXT_DIM = "#5B6F8C";
const BORDER_HI = "#24375A";

type D = NonNullable<HeroSignalData["governance_event"]>;

export default function HeroSignalGovernance({ d }: { d: D }) {
  const sigColor = d.significance === "critical" ? AMBER : TEAL_BRIGHT;

  return (
    <div style={{ position: "relative", display: "flex", flexDirection: "column", flex: 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <span style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.16em", color: TEXT_MUTED, textTransform: "uppercase" }}>
          Signal of the day
        </span>
        <span style={{
          padding: "3px 8px", fontFamily: MONO, fontSize: 10, letterSpacing: "0.1em",
          textTransform: "uppercase", borderRadius: 4,
          color: TEXT_MUTED, border: `1px solid ${BORDER_HI}`,
        }}>
          {d.body_name}
        </span>
      </div>

      <div style={{
        fontFamily: MONO, fontSize: 11, letterSpacing: "0.12em",
        color: sigColor, textTransform: "uppercase", marginBottom: 10,
      }}>
        Governance event approaching
      </div>

      <h2 style={{
        fontFamily: DISPLAY, fontWeight: 800, fontSize: 28,
        lineHeight: 1.2, letterSpacing: "-0.02em", marginBottom: 20, color: TEXT,
      }}>
        {d.title}
      </h2>

      <div style={{
        display: "flex", alignItems: "baseline", gap: 10, marginBottom: 16,
      }}>
        <span style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: 48, lineHeight: 1, color: AMBER }}>
          {d.days_until}
        </span>
        <span style={{ fontFamily: MONO, fontSize: 13, color: TEXT_MUTED }}>days away</span>
      </div>

      <div style={{ fontSize: 14, color: TEXT, lineHeight: 1.55, marginBottom: 12 }}>
        {d.description}
      </div>

      <div style={{
        fontFamily: MONO, fontSize: 10.5, color: TEXT_MUTED,
        marginBottom: 20,
      }}>
        {new Date(d.date).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long", year: "numeric" }).toUpperCase()}
      </div>

      <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
        <button className="mob-tap" style={{
          fontFamily: "'DM Sans'", fontSize: 12.5, fontWeight: 600,
          padding: "8px 14px", borderRadius: 7, border: "none", cursor: "pointer",
          background: TEAL, color: "#0A1628",
        }}>Prepare</button>
        <button className="mob-tap" style={{
          fontFamily: "'DM Sans'", fontSize: 12.5, fontWeight: 500,
          padding: "8px 14px", borderRadius: 7, cursor: "pointer",
          background: "transparent", color: TEXT_MUTED, border: `1px solid ${BORDER_HI}`,
        }}>View calendar</button>
        <button className="mob-tap" style={{
          fontFamily: "'DM Sans'", fontSize: 12.5, fontWeight: 500,
          padding: "8px 14px", borderRadius: 7, border: "none", cursor: "pointer",
          background: "transparent", color: TEXT_DIM, marginLeft: "auto",
        }}>Dismiss</button>
      </div>
    </div>
  );
}
