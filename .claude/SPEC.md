...
## Known issues / debt
- **scraper-ngo-reports — KNOWN BROKEN 2026-05-13.** Fetches NGO source pages successfully (e.g. 193k chars from WDC) but extracts 0 report links across all sources. `extractReportsFromMarkdown` regex requires `[text](url)` format with text matching `report|publication|brief` and text length ≥ 10 chars. Modern JS-rendered NGO pages publish report links as bare `*.pdf` URLs or short-text `[Download](url)` patterns that the current regex misses. NOT on cron schedule, so silently broken state is contained. Redesign required before re-enabling: multi-pattern extractor handling bare PDF URLs + JSON-LD structured data + short-text link variants. Also: OceanCare source URL is wrong (`/en/stories-and-news/` → should be `/en/about-oceancare/annual-reports/`).
