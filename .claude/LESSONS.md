...
## Word processor / draft editor patterns (2026-05-21)

- **React `stopPropagation` does not stop native bubbling to document listeners**: Since React 17+, event delegation lives at the React root element, not `document`. `e.stopPropagation()` on a synthetic event stops propagation *within React's virtual tree only* — the underlying native click event continues bubbling past the React root up to `document`. Any `document.addEventListener("click", ...)` handler will still fire. Click-outside patterns that rely on `e.stopPropagation()` from the toggle button will open-then-immediately-close the dropdown. Fix: add a `ref` to the toggleable container, and in the document handler bail out with `if (ref.current?.contains(e.target as Node)) return`. Applies to: dropdown menus, colour pickers, popovers, any element with a document-level "close on outside click" handler.
- **Code-read audits cannot verify runtime behaviour**: The Phase 1 audit marked menus and font size controls as "wired/works" based on reading handler code. Both were silent no-ops at runtime (menu closes instantly, `setMark` call is silently ignored). Future audits must include a runtime-test column — "opened it, clicked it, confirmed it did the thing" — not just a code-read column. "Handler exists" ≠ "handler fires". Audit verifications are themselves subject to Rule 4.
- **URL segment is project NAME, not UUID**: `app/platform/(shell)/projects/[id]/draft/page.tsx` receives `params.id` which is the project name (URL-encoded), not the UUID. `project_drafts.project_id` is a UUID FK. Passing the name to `.eq("project_id", name)` causes a silent PostgreSQL UUID parse error and returns empty rows — save and load silently fail. Fix: on mount, fetch `GET /api/projects/${name}` to retrieve `d.project_id` (the real UUID), store it in a `useRef`, and use it for all subsequent draft API calls.
- **Hardcoded placeholder values in production UI are a symptom of silent upstream failure**: if a component shows static values ("94%", "None") where dynamic data should be, check whether the upstream fetch is failing silently. The draft integrity card's hardcoded rows existed precisely because the UUID routing bug prevented the project fetch from returning real data. Hardcoded placeholders in production UI always warrant a fetch investigation before being accepted as intentional.
- **`editor.getText()` vs `editor.getHTML()`**: `getText()` strips all formatting marks on save — headings become plain text, bold disappears, lists collapse. Use `getHTML()` to persist rich content. On load, detect HTML vs plain-text with `raw.trimStart().startsWith("<")` to support backward-compatible loading of any pre-existing plain-text drafts.
- **Tiptap highlight `isActive` bug template**: `isActive("highlight") ? "#FBBC04" : "#FBBC04"` — same value both branches. This always renders the highlight indicator active regardless of cursor position. The inactive branch must be a distinct value (`"transparent"` for background, default colour for text).

## Ocean-relevance gate + source architecture (2026-05-28)

- **`OCEAN_DEDICATED_SOURCES` only bypasses keyword filter, NOT the Haiku ocean-gate**: a source in the set skips title-keyword matching but still runs through `checkOceanRelevance()`. Carbon market registry items (Verra: "ICVCM announces assessment decisions", VCMI: "New guide for carbon market innovations") correctly fail the gate — they don't mention ocean/marine. Add `skipGate: true` to `RSSSource` for sources that are definitionally Tideline-relevant but whose titles never contain ocean keywords.
- **Quarantine table is the first diagnostic for "source not landing" bugs**: when stories from a source don't appear in `stories`, check `stories_quarantine` before assuming a feed error. Items quarantined at 06:01 prove the cron ran and received the items — the gate is the blocker, not a fetch failure.
- **Carbon market classifier exclusions must name anti-patterns explicitly**: the initial `blue_carbon_credits` Haiku prompt excluded blue bonds and debt-for-nature swaps but said nothing about innovation challenges, science journalism, or general ocean climate commentary. Backfill produced 2/3 false positives from those categories. The tightened required test ("Is there a named entity taking a specific action on a specific instrument?") eliminated both false positives while keeping the 1 true positive (Verra wetland methodology).
- **Low tag count on a new tracker backfill is expected, not a signal of miscalibration**: blue_carbon_credits produced 1 genuine tag from 500 stories (60 days). Marine voluntary carbon markets are nascent — registry-level transactions are sparse. A 1-in-500 precision signal is accurate, not broken.
- **`skipGate` is a scalpel, not a bypass policy**: only use for sources where every item is definitionally relevant to Tideline's domain regardless of title wording (standards bodies, registries, official treaty bodies). Do not add `skipGate` to trade press or general media — keyword filter + gate is the right path for those.

## Source classification allowlists (2026-06-16)

- **Multilingual org name variants include parenthetical suffixes**: when a DB source_organisation is `"Organización de las Naciones Unidas para la Alimentación y la Agricultura (FAO)"`, the `(FAO)` is part of the string, not cosmetic. Allowlist entries must match the exact DB value including any parenthetical suffix. The same applies to Russian: `"...Объединенных Наций (ФАО)"`. Always dump the full string from the DB before finalising allowlist entries — never guess based on the "official name".
- **Codepoint-level hex dumps are the correct diagnostic for multilingual match failures**: when an exact-string match fails on a non-ASCII org name, the culprit is almost always a suffix mismatch (`(FAO)` vs nothing), not an encoding issue. Print `[...org].slice(0,40).map(c => '\\u' + c.codePointAt(0).toString(16).padStart(4,'0')).join('')` to confirm the exact stored codepoints. This rules out invisible Unicode normalisation differences before you go chasing encoding bugs.
- **Post-run verification block is not optional**: the `--patch` mode patch run completed "successfully" with 8 NULLs instead of 5. Without the verification block in the script that dumps remaining NULLs and their rule_applied, those 3 FAO mismatches would have been silently accepted. Every bulk classification script must query and print its own residuals at the end.
