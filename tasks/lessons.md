...
### Duplicate Sources of Truth for Config Data (FOURTEENTH instance)

- **FOURTEENTH instance -- low-stakes version.** Duplicate sources of truth for the same data (`TRACKER_LABELS` defined in both `app/lib/brief/utils.ts` and `app/lib/tracker-metadata.ts`) drifted such that the wrong source was being read. "Marine Species" shipped to subscribers for the cites-marine tracker because `utils.ts` was stale while `tracker-metadata.ts` had the correct "CITES Marine" label. `select.ts` imports from `utils.ts`, not `tracker-metadata.ts`, so the stale version was always active. Same class of failure as velocity.ts querying wrong field, local-vs-deployed prompt hash mismatch, etc. **RULE reinforced: any config or label data with multiple definitions is a latent drift bug.** The two definitions will diverge over time whenever one is updated without the other. Audit periodically for duplicate-source-of-truth patterns; consolidate when found. The fix (pick one file as canonical, delete the other, update all consumers) must happen together in a single atomic change -- never patch just one instance.

### Doc-vs-System Cadence Mismatch (FIFTEENTH instance)

- **FIFTEENTH instance, structural variant.** The Pulse Score methodology document specified weekly Monday cadence in two places (Sections 1 and 8) and the cron config ran every 4 days (`0 6 */4 * *`). The system was correct in itself — scores computed accurately, brief read them correctly, no broken code — but the doc-vs-system mismatch was silently shipping for months without anyone noticing until the brief render exposed it via score variation subscribers could see. **RULE: any external-facing claim about system behaviour (methodology documents, marketing copy, support documentation) must have a reconciliation check against actual config.** A drift between published cadence and actual cadence is just as serious as a drift between local and deployed code, but with worse credibility cost because it is visible to subscribers and citable by critics. The thirteenth lesson (verify production matches local) generalizes: verify published documentation matches actual system configuration, periodically and automatically where possible.

## 2026-05-27

### Silent Cron Failure — Timeout + End-of-Function Logging (SIXTEENTH instance)

- **SIXTEENTH instance — the most expensive class of silent failure: a cron that appears to run but produces zero output, with no error trace in any log.** Root cause: two compounding problems. (1) `cron_log.insert` was placed AFTER the 50-story processing loop. Vercel's 60-second function limit kills the process during the loop, so the log write is never reached. Zero entries in cron_log is not "no runs" — it is "every run timed out before the log write." (2) `generateEmbedding()` in `app/lib/embeddings.ts` had no `AbortSignal.timeout()`. When called from entity-matching Pass 3, a slow Jina embeddings response causes the entire serverless function to hang until Vercel kills it. **RULES: (a) Write diagnostic instrumentation (cron_log, progress counters) BEFORE the processing loop — or use try/finally — so partial runs are always recorded. (b) Every outbound `fetch()` call must have an `AbortSignal.timeout()`. No exceptions. A missing timeout on any single API call can brick an entire cron pipeline. (c) When a function performs N independent units of work (stories, rows, documents), check that N × average_unit_time < maxDuration. If it doesn't fit, either increase maxDuration (add a specific entry in vercel.json) or reduce N. Bump from 20→50 without bumping maxDuration from 60→300 was a capacity bug that silently broke the pipeline for 4 weeks.**
