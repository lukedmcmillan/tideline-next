...
- **THIRTEENTH adjacent finding (not a new lesson, a sharpening of the twelfth): the redesigned category gate had not actually been deployed.** The local working copy contained the gate; production was running the verb-era code with no gate at all. Brief outputs being interpreted as "the category gate selected X" were in fact "selectLeadFallback selected X under old significance-only logic with the headline-coherence fix on top." DEPLOY HYGIENE RULE: when a redesigned mechanism reports "complete" after a code change, immediately verify production is running that code, not the prior version. Specifically: the prompt-version hash exposed at runtime (in checkpoint output or a /health endpoint) must match the local prompt-version hash before any production behaviour is attributed to the new mechanism. Local diagnostics tell you what the local code does; production briefs tell you what the deployed code does; these can differ silently for days when unstaged changes accumulate.
