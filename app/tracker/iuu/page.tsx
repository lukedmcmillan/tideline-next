...
interface Stats {
  ...
  vessels_blacklisted: number;
  port_state_alerts: number;
...
function StatCards({ stats }: { stats: Stats }) {
  const cards = [
    { label: "Active Investigations", value: String(stats.active_investigations), color: TEAL },
    { label: "Vessels Blacklisted", value: String(stats.vessels_blacklisted), color: BLUE },
    { label: "Port State Alerts", value: String(stats.port_state_alerts), color: MUTED },
    { label: "Enforcement Status", value: stats.enforcement_status, color: stats.enforcement_status === "Active" ? TEAL : "#d97706" },
  ];
  return (
...
export default function IUUTracker() {
  ...
  useEffect(() => {
    fetch("/api/iuu-status")
      .then((r) => r.json())
      .then((data) => setStats(data))
      .catch(console.error)
      .finally(() => setLoading(false));

    fetch("/api/tracker-events?slug=iuu&limit=20")
      .then((r) => r.json())
      .then((data) => setTrackerEvents(data.events || []))
      .catch(() => {});

    fetch("/api/stories?limit=5&tracker=iuu")
      .then((r) => r.json())
      ...
      <div style={{ background: NAVY, borderBottom: `3px solid ${BLUE}`, padding: "0 20px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", height: 48, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            ...
            <a href="/tracker/iuu" style={{ color: WHITE, fontSize: 13, fontFamily: SANS, fontWeight: 600, textDecoration: "none" }}>IUU Fishing</a>
      ...
      <div style={{ background: NAVY, padding: "48px 20px 52px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", fontFamily: SANS, maxWidth: 600, lineHeight: 1.7 }}>
            Illegal, unreported and unregulated fishing — flag state enforcement, port state measures, and vessel tracking
          </p>
        </div>
      </div>
