...
function StatCards({ stats }: { stats: Stats }) {
  const cards = [
    { label: "Active Licences", value: String(stats.active_licences), color: TEAL },
    { label: "States Sponsoring", value: String(stats.states_sponsoring), color: BLUE },
    { label: "Pending Applications", value: String(stats.pending_applications), color: MUTED },
    { label: "Moratorium Status", value: stats.moratorium_status, color: stats.moratorium_status === "Adopted" ? TEAL : "#d97706" },
  ];
  return (
...
export default function ISATracker() {
  ...
  useEffect(() => {
    fetch("/api/isa-status")
      .then((r) => r.json())
      .then((data) => setStats(data))
      .catch(console.error)
      .finally(() => setLoading(false));

    fetch("/api/tracker-events?slug=isa&limit=20")
      .then((r) => r.json())
      .then((data) => setTrackerEvents(data.events || []))
      .catch(() => {});

    fetch("/api/stories?limit=5&tracker=isa")
      .then((r) => r.json())
      ...
      <div style={{ background: NAVY, borderBottom: `3px solid ${BLUE}`, padding: "0 20px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", height: 48, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
            ...
            <a href="/tracker/isa" style={{ color: WHITE, fontSize: 13, fontFamily: SANS, fontWeight: 600, textDecoration: "none" }}>Deep-Sea Mining</a>
      ...
      <div style={{ background: NAVY, padding: "48px 20px 52px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", fontFamily: SANS, maxWidth: 600, lineHeight: 1.7 }}>
            International Seabed Authority — licensing, moratorium debates, and environmental standards
          </p>
        </div>
      </div>
